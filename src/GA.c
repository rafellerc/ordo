...
int compare_GA(const void *a, const void *b){
...
void allocateChrom(Chrom *chrom, int jobNum, int** durees,int* h,int* H) {
...
Chrom* crossover(Chrom* par1, Chrom* par2, int len){
	...
	allocateChrom(childrens + 0, len, par1->durees,par1->h, par1->H);
	allocateChrom(childrens + 1, len, par1->durees,par1->h, par1->H);
...
Chrom mutation(Chrom* par, int len){
	Chrom child;
	allocateChrom(&child, len, par->durees,par->h, par->H);
	short bitTemp;
...
void selection(Chrom* population){
	qsort((void *)population, 2*P_SIZE, sizeof(Chrom), compare_GA);
...
void calFitness(Chrom* chrom){
	...
	for(int i = 0;i < jobNum;i++){
		// ex->durees[i] == NULL
		jobOrderList[i].opDurations[0] = chrom->durees[(*chrom).bits[i]][0];
		jobOrderList[i].opDurations[1] = chrom->durees[(*chrom).bits[i]][1];
		jobOrderList[i].opDurations[2] = chrom->durees[(*chrom).bits[i]][2];
		jobOrderList[i].totalDuration = chrom->durees[(*chrom).bits[i]][0] + chrom->durees[(*chrom).bits[i]][1] + chrom->durees[(*chrom).bits[i]][2];
		jobOrderList[i].index = (*chrom).bits[i];
	}

	//do the ordonnance for the first machine
	for(int i = 0,t = 0;i < jobNum;i++){
		if(notCollisional_XUE(t,jobOrderList[i].opDurations[0],h[0],H[0])){
	...
	for(int i = 0,t = 0;i < jobNum;i++){
		t = max_NEH(t,jobOrderList[i].operationStartTime[0] + jobOrderList[i].opDurations[0]);
	...
	for(int i = 0,t = 0;i < jobNum;i++){
		t = max_NEH(t,jobOrderList[i].operationStartTime[1] + jobOrderList[i].opDurations[1]);
...
int geneticAlgo(int** durees,int* h,int* H,int N_JOBS,int N_MACHINES){
	...
	for(int i = 0;i < 2 * P_SIZE;i++)
		allocateChrom(&population[i], jobNum, durees, h, H);
